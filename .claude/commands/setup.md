...
### Step 7.5: Google Calendar 連携（オプション）

「商談候補日を Google Calendar の空き時間から自動取得する機能を使いますか？（任意）」と確認する。

使わない場合はスキップ。候補日は `/email-workflow` の Step 5.5 で手動入力できる旨を案内する。
...
if creds.exists():
    ...
    if token.exists():
        print('[!!] token.json が存在します（既存の認証情報）')
        print('  → calendar.readonly スコープを追加するため、token.json の削除と再認証が必要です')
    else:
        print('[OK] token.json なし（/email-workflow 初回実行時にブラウザ認証が走ります）')
else:
    print('[NG] credentials/credentials.json が見つかりません')
    print('  → Step 8 で Gmail 認証と合わせて設定してください')
"
```
...
```
Google Calendar 連携の準備チェックリスト:

  ✅ Google Cloud Console で Calendar API を有効化済み
     （APIとサービス → ライブラリ → Google Calendar API → 有効にする）

  ✅ OAuth 同意画面のスコープに calendar.readonly を追加済み
     （APIとサービス → OAuth同意画面 → スコープを追加または削除）

  ✅ credentials/token.json を削除済み（既存がある場合のみ）
     （スコープ変更後は必ず削除して再認証が必要）

これらが完了したら、/email-workflow を実行すると自動的にブラウザ認証が走ります。
```

**token.json の削除が必要な場合：**
...
if token.exists():
    token.unlink()
    print('[OK] token.json を削除しました。次回 /email-workflow 実行時に再認証が行われます。')
else:
    print('[OK] token.json は存在しません（削除不要）')
"
```

cli_config.yaml の calendar.enabled が true になっていることを確認する：
...
print('calendar.enabled:', cal.get('enabled', True))
print('calendar.days_ahead:', cal.get('days_ahead', 14))
print('calendar.duration_minutes:', cal.get('duration_minutes', 60))
print('calendar.max_slots:', cal.get('max_slots', 5))
"
```
...
if 'calendar' not in cfg:
    cfg['calendar'] = {}
cfg['calendar']['enabled'] = True    # 無効にする場合は False
cfg['calendar']['days_ahead'] = 14   # 何日先まで探すか
cfg['calendar']['duration_minutes'] = 60  # 商談の想定時間（分）
cfg['calendar']['max_slots'] = 5     # 最大何件提案するか
save_cli_config(cfg)
print('Calendar 設定を保存しました')
"
```
