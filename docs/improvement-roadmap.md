...
**バージョン**: 2026年5月  
**現在地**: Phase 6 完了 → Phase 7（/setup Skill 新設）準備中
...
| **Phase 6** | 全体改善まとめ（§3.2〜§3.16, §3.5'） | Phase 5 で積み上げた改善バックログを実装。Python機械チェック・LLM-as-a-Judge・3層検証アーキテクチャ・マッピング辞書拡充（実LeadManagerCSV対応含む）・Whisper精度改善・output世代管理・BM25警告解消等 10実装。テスト数 52→117件。 |
| **製品カード機能** | run_kb_summary・/kb-verify・match_product_cards(ファジー)・RAG上への注入・テスト20件 | 完了 |
...
### 3.17 商談確度の値フォーマット正規化

**背景**: 実LeadManagerCSVの「商談確度」カラムは「B：担当者フォロー」のようにランク記号と
説明テキストが混在した形式。現状は文字列ごと lead_rank に入るため、ランク判定ロジック
（rank_estimator等）が "B" を正しく認識できない可能性がある。

**想定実装**: `apply_column_mapping()` またはランク正規化関数で、lead_rank フィールドの
値から先頭の A〜E 記号を抽出する後処理を追加。例: `"B：担当者フォロー"` → `"B"`

**優先度**: 中（Phase 8 の一気通貫テストで実CSV使用時に顕在化する可能性）

### 3.18 フォロー担当 / スキャン担当者の分離

**背景**: 実LeadManagerCSVに「フォロー担当」（営業担当）と「スキャン担当者」（スキャナー操作者）
の2カラムが存在し、現状はどちらも rep_name にマップされ先着一致になる。音声紐づけ
（/audio-matching）の精度は「スキャン担当者」に依存するため、フォロー担当との混在が
誤マッチを引き起こす可能性がある。

**想定実装**: OPTIONAL_FIELDS に `follow_person`（フォロー担当）を新設し、`rep_name` を
スキャン担当者専用に限定する。または優先度付きマッピングを実装。

**優先度**: 中（音声機能強化と合わせて対応）

### 3.19 大規模KBのmap-reduce要約

**背景**: run_kb_summary は全資料を一度にLLMへ渡すため、巨大なPDFが多数ある
KBでは context長・コストが問題になりうる。

**想定実装**: 文書ごとに要約→統合（map-reduce）に切り替え、規模で自動分岐。

**優先度**: 低（小規模KBでは不要。現状は一度きりの処理なので影響限定的）。
...
*更新日: 2026年6月7日（製品カード機能完了・§3.19 追記）*
