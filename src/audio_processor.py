...
temperature の判定基準:
・high（熱意高）: 以下を2つ以上満たす場合
  - 予算が具体的な金額で確認済み（例:「400万円」「〜万の予算あり」）
  - 意思決定権が本人にある（「稟議不要」「自分で決められる」「上申不要」）
  - 次アクションへの明確な意欲（「デモ希望」「来週見たい」「今月中に」）
・warm（検討中）: 興味あり・検討中だが予算/承認が不明
・cold（情報収集）: まだ情報収集段階・次アクションへの意欲なし

必ず上記基準に従ってください。high の条件を2つ以上満たす場合は必ず high にしてください。
...
出力フォーマット（JSONのみ、他のテキスト不要）:
{
  ...
  "temperature": "商談温度感（high/warm/cold）"
}"""
